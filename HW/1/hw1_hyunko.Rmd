...
```{r}
new_covid <- new_covid %>% mutate(death.rate = deaths / cases)
head(new_covid, 6)
```
...
state.level <- state.level %>%
  mutate(state.rate = tot_deaths / tot_cases)
state.level
```

(c) What is the case fatality rate in Virginia?

** A fatality rate of Virginia is 0.01659071. **
```{r}
...
```{r}
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, xlab = "Flipper Length (in mm)", ylab = "Body Mass (in gram)", title = "A Scatterplot of Flipper Length vs Body Mass")) +
  geom_point()
```
...
** The slope implies flipper length is proportional to body mass.**

(g) Does the estimated intercept make sense contextually?

** The intercept is abnormally small.**
...
- P-Value Approach: As P-value is 2.2e-16 < 0.05, so we can reject the null hypothesis.
...
** Estimate mean body mass = 1 4158.88, 95% CI: (3978.164, 4339.596)**
```{r}
